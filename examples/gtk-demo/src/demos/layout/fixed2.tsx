...
import { FixedChild, GtkBox, GtkButton, GtkFixed, GtkFrame, GtkLabel, GtkScale, GtkScrolledWindow } from "@gtkx/react";
...
.transform-widget.accent {
    background-color: @accent_bg_color;
    color: @accent_fg_color;
}

.transform-widget.success {
    background-color: @success_bg_color;
    color: @success_fg_color;
}

.transform-widget.warning {
    background-color: @warning_bg_color;
    color: @warning_fg_color;
}

.transform-widget.error {
...
.z-back { background-color: alpha(@accent_color, 0.2); }
.z-middle { background-color: alpha(@accent_color, 0.4); }
.z-front { background-color: alpha(@accent_color, 0.6); }
`;
        try {
            ...
            >
                ...
                <GtkFrame label="Transformed Widgets">
                    <GtkBox
                        ...
                        marginBottom={16}
                    >
                        <GtkFixed widthRequest={400} heightRequest={250} cssClasses={["fixed-container"]}>
                            {widgets.map((widget) => (
                        ...
                        <GtkBox orientation={Gtk.Orientation.HORIZONTAL} spacing={8} halign={Gtk.Align.CENTER}>
                            {widgets.map((w, i) => (
                                <GtkLabel
                                    key={w.id}
                                    label={`${w.label} (z=${i})`}
                                    cssClasses={["z-layer", i < 2 ? "z-back" : i < 4 ? "z-middle" : "z-front"]}
                                />
                            ))}
                        </GtkBox>
