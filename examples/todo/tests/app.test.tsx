...
describe("Todo App", () => {
    ...
    describe("filtering todos", () => {
        it("shows filter bar when todos exist", async () => {
            ...
            await expect(screen.findByName("filter-all", { timeout: 100 })).rejects.toThrow();

            const input = await screen.findByName("todo-input");
            const addButton = await screen.findByName("add-button");
            await addTodo(input, addButton, "Test todo");

            const filterAll = await screen.findByName("filter-all");
        ...
        it("filters to show only active todos", async () => {
            ...
            const filterActive = await screen.findByName("filter-active");
        ...
        it("filters to show only completed todos", async () => {
            ...
            const filterCompleted = await screen.findByName("filter-completed");
    ...
    describe("clear completed", () => {
        it("shows clear completed button when there are completed todos", async () => {
            ...
            await expect(screen.findByName("clear-completed", { timeout: 100 })).rejects.toThrow();

            const checkbox = await screen.findByRole(Gtk.AccessibleRole.CHECKBOX);
            await userEvent.click(checkbox);

            const clearButton = await screen.findByName("clear-completed");
