...
const GesturesDemo = () => {
    const [_clickInfo, _setClickInfo] = useState<{
        x: number;
        y: number;
        nPress: number;
    } | null>(null);
    const [_dragInfo, _setDragInfo] = useState<{
        startX: number;
        startY: number;
        offsetX: number;
        offsetY: number;
    } | null>(null);
    const [_swipeInfo, _setSwipeInfo] = useState<{
        velocityX: number;
        velocityY: number;
        direction: string;
    } | null>(null);
    const [_gestureHistory, setGestureHistory] = useState<string[]>([]);

    const _addToHistory = (message: string) => {
        setGestureHistory((prev) => [...prev.slice(-5), message]);
    };

    const _getSwipeDirection = (vx: number, vy: number): string => {
