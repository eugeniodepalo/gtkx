...
describe("Exhaustive Integer Type Permutations", () => {
    describe("8-bit integers", () => {
        it("should handle 8-bit unsigned integers (u8)", () => {
            ...
            // '0' = 48, '9' = 57
            const result = call(GLIB_LIB, "g_ascii_digit_value", [{ type: { type: "int", size: 8, unsigned: true }, value: 48 }], {
                type: "int",
                size: 32,
            });
            expect(result).toBe(0);
        ...
        it("should handle 8-bit signed integers (i8)", () => {
            const result = call(GLIB_LIB, "g_ascii_digit_value", [{ type: { type: "int", size: 8, unsigned: false }, value: 57 }], {
                type: "int",
                size: 32,
            });
        ...
        it("should handle 8-bit boundary values", () => {
            // Use g_ascii_isalnum which takes a char
            // 'A' = 65, 'z' = 122
            const resultA = call(GLIB_LIB, "g_ascii_isalnum", [{ type: { type: "int", size: 8, unsigned: true }, value: 65 }], {
                type: "int",
                size: 32,
            });
            expect(resultA).not.toBe(0);

            const resultSpace = call(GLIB_LIB, "g_ascii_isalnum", [{ type: { type: "int", size: 8, unsigned: true }, value: 32 }], {
                type: "int",
                size: 32,
            });
            expect(resultSpace).toBe(0);
        });

        it("should handle 8-bit max unsigned value (255)", () => {
            const result = call(GLIB_LIB, "g_ascii_isprint", [{ type: { type: "int", size: 8, unsigned: true }, value: 255 }], {
                type: "int",
                size: 32,
            });
    ...
    describe("32-bit integers", () => {
        it("should handle 32-bit unsigned integers (u32)", () => {
            const quark = call(
                GLIB_LIB,
                "g_quark_from_string",
                [{ type: { type: "string" }, value: "test-u32" }],
                { type: "int", size: 32, unsigned: true },
            ) as number;
            expect(quark).toBeGreaterThan(0);
        });
    ...
    describe("64-bit integers", () => {
        ...
        it("should handle 64-bit large positive values", () => {
            const time1 = call(GLIB_LIB, "g_get_monotonic_time", [], { type: "int", size: 64, unsigned: false }) as number;
            const time2 = call(GLIB_LIB, "g_get_monotonic_time", [], { type: "int", size: 64, unsigned: false }) as number;
...
describe("Exhaustive Callback Trampoline Permutations", () => {
        ...
        it("should handle closure with no arguments", () => {
            ...
            const handlerId = call(
                ...
                [
                    ...
                    { type: { type: "string" }, value: "clicked" },
                    {
                        type: { type: "callback", trampoline: "closure", argTypes: [{ type: "gobject", borrowed: true }] },
                        value: () => {
                            called = true;
                        },
                    },
    ...
    describe("SourceFunc trampoline", () => {
        it("should handle sourceFunc callback for idle_add", async () => {
            let callCount = 0;
            ...
            call(
                ...
                "g_idle_add_full",
                [
                    { type: { type: "int", size: 32 }, value: 200 }, // G_PRIORITY_DEFAULT_IDLE
                    {
                        type: { type: "callback", trampoline: "sourceFunc" },
                        value: () => {
                            callCount++;
                            return callCount < 3; // Return true to continue, false to stop
                        },
                    },
                    {
                        type: { type: "callback", trampoline: "destroy" },
            ...
            // Process main context to trigger the idle callback
            for (let i = 0; i < 10; i++) {
                call(
                    GLIB_LIB,
                    "g_main_context_iteration",
        ...
        it("should handle sourceFunc with timeout_add", async () => {
            let called = false;
            ...
            call(
                ...
                "g_timeout_add_full",
                [
                    { type: { type: "int", size: 32 }, value: 200 }, // G_PRIORITY_DEFAULT
                    { type: { type: "int", size: 32, unsigned: true }, value: 1 }, // 1ms timeout
                    {
                        type: { type: "callback", trampoline: "sourceFunc" },
                        value: () => {
                            called = true;
            ...
            const startTime = Date.now();
            while (!called && Date.now() - startTime < 500) {
                call(
                    GLIB_LIB,
                    "g_main_context_iteration",
                    [
...
describe("Exhaustive Null and Undefined Permutations", () => {
    describe("Null type", () => {
        ...
        it("should handle null replacing GObject argument", () => {
            ...
            const child = call(GTK_LIB, "gtk_window_get_child", [{ type: { type: "gobject" }, value: window }], {
                type: "gobject",
                borrowed: true,
            });
