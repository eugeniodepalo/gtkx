...
impl TryFrom<arg::Arg> for Value {
    ...
    fn try_from(arg: arg::Arg) -> anyhow::Result<Value> {
        match &arg.type_ {
            ...
            Type::Callback => Value::try_from_callback(&arg),
...
impl Value {
    ...
    fn try_from_callback(arg: &arg::Arg) -> anyhow::Result<Value> {
        ...
        let closure = glib::Closure::new(move |args: &[glib::Value]| {
            let args_values: Vec<value::Value> = args.iter().map(Into::into).collect();
