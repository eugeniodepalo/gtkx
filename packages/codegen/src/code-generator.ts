...
import { TypeMapper, FfiTypeDescriptor } from "./type-mapper.js";
...
export class CodeGenerator {
  ...
  constructor(private options: GeneratorOptions) {
    this.typeMapper = new TypeMapper(options.namespace);
  }
  ...
  async generateNamespace(namespace: GirNamespace): Promise<Map<string, string>> {
    ...
    for (const iface of namespace.interfaces) {
      ...
      const content = await this.generateInterface(iface, namespace.sharedLibrary);
    ...
    for (const cls of namespace.classes) {
      ...
      const content = await this.generateClass(cls, namespace.sharedLibrary, classMap);
      files.set(fileName, content);
    }
    ...
    for (const func of namespace.functions) {
      ...
      if (match && func.returnType.name) {
        const potentialClassName = match[1].split('_').map(s => 
    ...
    if (standaloneFunctions.length > 0) {
      const functionsContent = await this.generateFunctions(standaloneFunctions, namespace.sharedLibrary);
  ...
  private async generateClass(cls: GirClass, sharedLibrary: string, classMap: Map<string, GirClass>): Promise<string> {
    const imports = this.typeMapper.generateTypeImport();
    ...
    const extendsClause = cls.parent && classMap.has(cls.parent)
    ...
    if (cls.constructors.length > 0) {
      ...
      const mainConstructor = cls.constructors.find(c => 
        !c.parameters.some(p => p.name === "..." || p.name === "")
      );
      
      if (mainConstructor) {
        // Use the first non-varargs constructor as the main constructor
        const hasParent = !!(cls.parent && classMap.has(cls.parent));
        code += this.generateConstructor(mainConstructor, cls.name, sharedLibrary, hasParent);
        code += "\n";
        
        // Generate static factory methods for all other constructors
        for (const constructor of cls.constructors) {
          if (constructor !== mainConstructor) {
            code += this.generateStaticFactoryMethod(constructor, cls.name, sharedLibrary);
            code += "\n";
          }
        }
      } else {
  ...
  private generateStaticFactoryMethod(constructor: GirConstructor, className: string, sharedLibrary: string): string {
    // Extract method name from c_identifier
    // e.g., g_object_new_valist -> newValist
    let methodName = "new";
    if (constructor.cIdentifier) {
      const parts = constructor.cIdentifier.split("_");
    ...
    const params = this.generateParameterList(constructor.parameters);
    
    let code = `  static ${methodName}(${params}): ${className} {\n`;
    code += `    const ptr = call(\n`;
    code += `      "${sharedLibrary}",\n`;
    code += `      "${constructor.cIdentifier}",\n`;
    code += `      [\n`;
    ...
    code += `    );\n`;
    code += `    const instance = Object.create(${className}.prototype) as ${className};\n`;
    code += `    (instance as any).ptr = ptr;\n`;
  ...
  private generateConstructor(constructor: GirConstructor, className: string, sharedLibrary: string, hasParent: boolean = false): string {
    // Always make constructor parameters optional to allow derived classes to call super()
    const params = this.generateParameterList(constructor.parameters, true);
  ...
  private async generateInterface(iface: GirInterface, sharedLibrary: string): Promise<string> {
    const imports = this.typeMapper.generateTypeImport();
  ...
  private generateMethod(method: GirMethod, sharedLibrary: string, className?: string): string {
    ...
    if (className && methodRenames.has(className)) {
      ...
      if (renames.has(methodName)) {
        methodName = renames.get(methodName)!;
      }
    }
  ...
  private generateProperty(property: GirProperty, className: string, sharedLibrary: string): string {
    // Convert property name to valid JavaScript identifier
    const propertyName = this.toValidIdentifier(this.toCamelCase(property.name));
    const typeMapping = this.typeMapper.mapType(property.type);
    let code = "";

    // Generate getter
    if (property.readable) {
      const getterName = `get${this.toPascalCase(property.name)}`;
      const cGetterName = `g_object_get_property`; // This would need proper C function name
    ...
    if (property.writable && !property.constructOnly) {
      ...
      const cSetterName = `g_object_set_property`; // This would need proper C function name
  ...
  private generateSignalConnect(sharedLibrary: string, hasConnectMethod: boolean = false): string {
  ...
  private generateParameterList(parameters: GirParameter[], makeAllOptional: boolean = false): string {
    const filteredParams = parameters
      .filter(param => param.name !== "..." && param.name !== ""); // Filter out varargs
  ...
  private generateParameterTypes(parameters: GirParameter[]): string[] {
  ...
  private generateCallArgument(paramName: string, ffiType: FfiTypeDescriptor): string {
  ...
  private toValidIdentifier(str: string): string {
    ...
    const reservedWords = new Set([
      ...
      "let", "static", "enum", "implements", "interface", "package",
